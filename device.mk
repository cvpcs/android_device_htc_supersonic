...
# properly on htc supersonic hardware
#
...
$(foreach prop,$(USE_PROPRIETARIES), \
  $(if $(wildcard device/htc/supersonic/proprietary.$(prop)), \
    $(eval \
PRODUCT_COPY_FILES += $(shell \
        cat device/htc/supersonic/proprietary.$(prop) \
        | sed -r 's/^\/(.*\/)([^/ ]+)$$/device\/htc\/supersonic\/proprietary\/\2:\1\2/' \
        | tr '\n' ' ') \
     ), \
    $(error Cannot include proprietaries from $(prop). List file device/htc/supersonic/proprietary.$(prop) does not exist) \
   ) \
 )
endif
